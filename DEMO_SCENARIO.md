...
3. 추가 질문:
   ...
   → **다른 방에 있는** 메모리까지 그래프로 탐색:
     ...
     - "박관리는 ISO 9001:2015 체크리스트를 사용" (개인 대화방)
   → **참조 소스** 패널에서 소스가 여러 대화방(프로젝트 대화방 + 개인 대화방)에서 온 것을 확인

> **핵심 포인트**: "박관리"를 검색하면 단순 키워드 매칭이 아니라, 엔티티 그래프를 따라가며 관련 프로젝트·미팅·개인 대화방 메모까지 연결하여 포괄적인 답변 생성
...
## 데모 7: Mchat(Mattermost) 연동 (3분)

> **목적**: Mattermost 채널과 연동하여 메시지를 자동 저장하고, 멤버를 실시간 동기화하는 것을 보여줌
...
- **모든 메시지 자동 저장**: `@ai` 멘션 없이도 채널의 모든 메시지가 메모리에 자동 저장됨
- **멤버 실시간 동기화**: WebSocket 이벤트를 통해 채널 멤버 변경이 MemGate 대화방에 자동 반영
  - 채널에 사용자가 추가되면 대화방 멤버로 자동 등록
  - 채널에서 사용자가 나가면 대화방 메모리 접근 권한이 회수됨
- **시스템 메시지 필터링**: 입장/퇴장 등 시스템 메시지는 메모리에 저장하지 않음
- **봇 사용자 제외**: 봇 계정은 대화방 멤버에서 제외됨
- **사이드바 숨김**: Mchat 연동 대화방은 사이드바에 표시되지 않지만, 저장된 메모리는 "내 지식 목록"에서 확인 가능

---

## 데모 흐름 요약

> **메모리 스코프 정책**: MemGate는 2가지 스코프만 사용합니다.
> - **chatroom**: 대화방 메모리 (개인 대화방 포함). 개인 선호/사실 정보는 사용자의 개인 대화방(솔로룸)에 저장됩니다.
> - **agent**: 에이전트가 수집한 메모리
...
7. Mchat                   →  Mattermost 채널 동기화 + 메시지 자동 저장
```

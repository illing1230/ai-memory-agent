# Mchat 연동 검토 사항

## 1. 현재 상태

### 연결 성공 ✅
- **URL**: `https://mchat.mosaic.sec.samsung.net`
- **인증**: Bearer Token 방식
- **WebSocket**: 실시간 이벤트 수신 동작 확인

### 수신되는 이벤트
| 이벤트 | 설명 | 상태 |
|--------|------|------|
| `hello` | WebSocket 연결 성공 | ✅ |
| `channel_viewed` | 채널 조회 | ✅ |
| `posted` | 새 메시지 | ✅ |
| `typing` | 타이핑 중 | ✅ |

---

## 2. 아키텍처 결정 사항

### 2.1 연동 방식: WebSocket vs Webhook

| 항목 | WebSocket (현재) | Outgoing Webhook |
|------|-----------------|------------------|
| 연결 방식 | 상시 연결 유지 | 요청 시에만 |
| 서버 부담 | 높음 | 낮음 |
| Private 채널 | ✅ 가능 | ❌ Public만 |
| DM | ✅ 가능 | ❌ 불가 |
| 스케일 아웃 | 어려움 | 쉬움 |
| 설정 | 코드만 | Mchat 관리자 필요 |

**현재 결정**: WebSocket 방식 사용 (Private 채널, DM 지원 필요)

### 2.2 메시지 처리 범위

```
┌─────────────────────────────────────────────────────────────────┐
│                    메시지 처리 정책                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  [수신 대상]                                                     │
│  ✅ 내 메시지 - 메모리 저장 (AI 응답은 안 함)                    │
│  ✅ 다른 사람 메시지 - 메모리 저장 + AI 응답                     │
│  ✅ @ai 멘션 - AI 응답 트리거                                    │
│  ✅ /remember - 명시적 메모리 저장                               │
│  ✅ /search - 메모리 검색                                        │
│                                                                  │
│  [저장 대상]                                                     │
│  ✅ 모든 메시지 → 대화 히스토리 저장                             │
│  ✅ 중요 정보 → 메모리 자동 추출                                 │
│  ❌ AI 응답 → 저장하지 않음 (또는 별도 표시)                     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 3. 구현 필요 사항

### 3.1 메시지 처리 로직

```python
# 메시지 수신 시 처리 흐름
async def handle_posted(event):
    post = event["data"]["post"]
    user_id = post["user_id"]
    message = post["message"]
    channel_id = post["channel_id"]
    
    # 1. 메시지 저장 (내 메시지 포함)
    await save_message_to_history(channel_id, user_id, message)
    
    # 2. 메모리 자동 추출 (중요 정보)
    memories = await extract_memories(message)
    if memories:
        await save_memories(memories, channel_id, user_id)
    
    # 3. AI 응답 (내 메시지는 제외)
    if user_id != BOT_USER_ID:
        if "@ai" in message or message.startswith("/"):
            response = await generate_ai_response(message, channel_id)
            await send_response(channel_id, response)
```

### 3.2 채널-대화방 매핑

```sql
-- Mchat 채널 ↔ AI Memory Agent 대화방 매핑
CREATE TABLE mchat_channel_mapping (
    id TEXT PRIMARY KEY,
    mchat_channel_id TEXT UNIQUE NOT NULL,
    mchat_channel_name TEXT,
    mchat_team_id TEXT,
    agent_room_id TEXT REFERENCES chat_rooms(id),
    sync_enabled BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

### 3.3 사용자 매핑

```sql
-- Mchat 사용자 ↔ AI Memory Agent 사용자 매핑
CREATE TABLE mchat_user_mapping (
    id TEXT PRIMARY KEY,
    mchat_user_id TEXT UNIQUE NOT NULL,
    mchat_username TEXT,
    agent_user_id TEXT REFERENCES users(id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```

---

## 4. 검토 필요 사항

### 4.1 Mchat 관리자 확인 필요

| 항목 | 상태 | 비고 |
|------|------|------|
| Bot Account 생성 | ⏳ 요청 필요 | 현재 개인 토큰 사용 중 |
| Bot Token 발급 | ⏳ 요청 필요 | |
| API Rate Limit | ❓ 확인 필요 | 기본값 10req/sec |
| 채널 접근 권한 | ❓ 확인 필요 | Bot 초대 방식 |

### 4.2 Bot Account 요청 사항

```
Bot Account 생성 요청:
- Username: ai-memory-bot
- Display Name: AI Memory Bot
- 아이콘: 🤖 또는 커스텀 이미지
- 권한:
  - 메시지 읽기/쓰기
  - 채널 참여
  - 사용자 정보 조회
```

### 4.3 보안 검토

- [ ] 토큰 저장 방식 (환경변수, Vault 등)
- [ ] 민감정보 필터링 (개인정보, 비밀번호 등)
- [ ] 로그에 토큰/민감정보 노출 방지
- [ ] 메시지 암호화 전송 확인 (HTTPS)

---

## 5. Streamlit + Mchat 통합 구조

```
┌─────────────────────────────────────────────────────────────────┐
│                      통합 아키텍처                               │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   [Streamlit UI :8501]                                           │
│   • 메모리 관리/검색                                             │
│   • 대시보드                                                     │
│   • 설정                                                         │
│        │                                                         │
│        ▼                                                         │
│   [FastAPI :8000]                                                │
│   • /api/v1/memories                                             │
│   • /api/v1/chat-rooms                                           │
│   • /api/v1/users                                                │
│        │                                                         │
│        ▼                                                         │
│   [SQLite + Qdrant] ◄────────────────────┐                       │
│   • messages 테이블                       │                       │
│   • memories 테이블                       │                       │
│   • 벡터 인덱스                           │                       │
│        ▲                                 │                       │
│        │                                 │                       │
│   [Mchat WebSocket Worker]               │                       │
│   • 실시간 메시지 수신 ──────────────────┘                       │
│   • 메모리 자동 추출                                             │
│   • AI 응답 전송                                                 │
│        │                                                         │
│        ▼                                                         │
│   [Samsung Mchat]                                                │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 실행 방식

```bash
# 방법 1: 별도 프로세스
uvicorn src.main:app --port 8000 &
python -m src.mchat.worker &
streamlit run app/streamlit_app.py --server.port 8501

# 방법 2: FastAPI 시작 시 WebSocket 워커 함께 실행
# (main.py에서 백그라운드 태스크로)
```

---

## 6. 다음 단계

### Phase 1: 기본 연동 (1주)
- [x] WebSocket 연결 테스트
- [x] 메시지 수신 확인
- [ ] 메시지 저장 로직 구현
- [ ] 채널/사용자 매핑 테이블 생성
- [ ] 기본 커맨드 연동 (@ai, /remember)

### Phase 2: 메모리 연동 (1주)
- [ ] Mchat 메시지 → Memory 저장
- [ ] 메모리 검색 → Mchat 응답
- [ ] 자동 메모리 추출

### Phase 3: 안정화 (1주)
- [ ] Bot Account 전환
- [ ] 에러 핸들링
- [ ] 재연결 로직 강화
- [ ] 모니터링/로깅

---

## 7. 환경변수

```env
# .env 파일
MCHAT_URL=https://mchat.mosaic.sec.samsung.net
MCHAT_TOKEN=sig14w8mj7naxnmdzghk4qa88w
MCHAT_ENABLED=true

# Bot Account 전환 시 (추후)
# MCHAT_TOKEN=<bot-token>
# MCHAT_BOT_USER_ID=<bot-user-id>
```

---

## 8. 참고 자료

- [Mattermost API v4 Reference](https://api.mattermost.com/)
- [Mattermost WebSocket Events](https://developers.mattermost.com/integrate/reference/websocket-events/)
- [Mattermost Bot Accounts](https://developers.mattermost.com/integrate/reference/bot-accounts/)
